  ...
  )
{
  ...
  for (Index = 0; Index < MemoryMapHob->Count; Index++) {
    MemMapTable = ((EFI_MEMORY_DESCRIPTOR *)MemoryMapHob->MemoryMapTable) + Index;
    Hob.Raw = GetHobList();
  ...
  )
{
  ...
  for (Index = 0; Index < MemoryMapHob->Count; Index++) {
    MemoryMapTable = ((EFI_MEMORY_DESCRIPTOR *)MemoryMapHob->MemoryMapTable) + Index;
  ...
  MemoryMapTable = ((EFI_MEMORY_DESCRIPTOR *)MemoryMapHob->MemoryMapTable) + FindIndex;
