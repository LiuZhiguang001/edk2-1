  ...
  )
{
  ...
  Cmdline = AllocatePages (EFI_SIZE_TO_PAGES ((UINTN)AsciiStrLen (Defaultcmdline)));
  CopyMem (Cmdline, Defaultcmdline, (UINTN)AsciiStrLen (Defaultcmdline));

  // struct linux_params params;
  params = AllocatePages (EFI_SIZE_TO_PAGES (sizeof (struct linux_params)));
  ...
  //
  NewKernelBase =AllocateAlignedPages (EFI_SIZE_TO_PAGES (params->init_size), params->kernel_alignment);
  ...
  )
{
  ...
  StackBase = (EFI_PHYSICAL_ADDRESS)(UINTN)AllocateAlignedPages (EFI_SIZE_TO_PAGES (STACK_SIZE), CPU_STACK_ALIGNMENT);
